...
export function Home() {
  ...
  return (
    <div>
      ...
      >
        <div className="w-16 h-16 rounded-[18px] mx-auto mb-5 flex items-center justify-center text-[28px] font-black text-white tracking-tight shadow-[0_8px_32px_rgba(232,93,38,.3)]" style={{ background: 'linear-gradient(135deg, var(--color-accent), #ff8a50)' }}>
          AI
        </div>
        <h1 className="text-[clamp(1.6rem,5vw,2.5rem)] font-extrabold leading-tight tracking-tight mb-2">
          AI for the{' '}
          <span className="bg-clip-text text-transparent" style={{ backgroundImage: 'linear-gradient(135deg, var(--color-accent), #ff8a50)' }}>
            Non-Dev
          </span>
        </h1>
        <p className="text-[15px] text-muted max-w-[420px] mx-auto">
          Learn to build with AI — explained like you're learning from a friend. Zero jargon. Real examples. Interactive quizzes.
        </p>
      </div>

      {/* Stats */}
      <div className="flex justify-center gap-7 px-6 pt-5 pb-2">
        <div className="text-center">
          <div className="text-[22px] font-extrabold text-accent">{allContent.length}</div>
          <div className="text-[11px] text-muted font-medium uppercase tracking-wider">Topics</div>
        </div>
        <div className="text-center">
          <div className="text-[22px] font-extrabold text-accent">{totalLessons}</div>
          <div className="text-[11px] text-muted font-medium uppercase tracking-wider">Lessons</div>
        </div>
        <div className="text-center">
          <div className="text-[22px] font-extrabold text-accent">{getTotalCompleted()}</div>
          <div className="text-[11px] text-muted font-medium uppercase tracking-wider">Completed</div>
        </div>
      </div>
